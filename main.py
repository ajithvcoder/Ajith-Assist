...
class AgenticAI:
    ...
    async def _create_execution_plan(self, analysis: Dict[str, Any], request: AgentRequest) -> List[AgentStep]:
        ...
        logger.info("Execution plan generated by LLM.")
    ...
    async def _execute_plan(self, plan: List[AgentStep], request: AgentRequest) -> str:
        ...
        for step in plan:
            ...
            logger.info(f"Executed Step {step.step_number}: confidence={confidence:.2f}")
    ...
    async def _evaluate_step_quality(self, result: str, step: AgentStep) -> float:
        ...
        try:
            ...
            logger.info(f"Evaluated step quality: {confidence:.2f} for step {step.step_number}")
    ...
    async def _improve_step_result(self, result: str, step: AgentStep, context: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"Improvement attempted for Step {step.step_number}")
    ...
    async def _synthesize_final_result(self, plan: List[AgentStep], request: AgentRequest) -> str:
        ...
        logger.info("Synthesized final result using LLM.")
    ...
    async def _self_evaluate_and_refine(self, result: str, request: AgentRequest) -> str:
        ...
        logger.info("Self-evaluation completed. Refinement decision made.")
    ...
    async def _call_gemini(self, prompt: str) -> str:
        ...
        try:
            # Log the prompt length and a safe preview
            logger.info(f"LLM Call Prompt Preview (first 300 chars): {prompt[:300].replace(chr(10), ' ')}")
            logger.debug(f"Full LLM Prompt: {prompt}")
            response = await self.model.generate_content_async(prompt)
            logger.info(f"LLM Response Preview (first 300 chars): {response.text[:300].replace(chr(10), ' ')}")
            logger.debug(f"Full LLM Response: {response.text}")
